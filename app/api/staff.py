...
async def assign_service_to_staff(staff_id: int, service_id: int, db: AsyncSession = Depends(get_db)):
    staff = await db.get(Staff, staff_id)
    if staff is None:
        raise HTTPException(status_code=404, detail="Staff not found")
...
async def unassign_service_from_staff(staff_id: int, service_id: int, db: AsyncSession = Depends(get_db)):
...
async def list_staff_schedule(staff_id: int, db: AsyncSession = Depends(get_db)):
    staff = await db.get(Staff, staff_id)
    if staff is None:
        raise HTTPException(status_code=404, detail="Staff not found")
...
async def delete_staff_schedule_day(staff_id: int, schedule_id: int, db: AsyncSession = Depends(get_db)):
